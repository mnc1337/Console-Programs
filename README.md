...
- **URL parser**: a tool that parses a given URL after defining content-type of page (`HTML` or `JSON`, i.e. `text/html` or `appliation/json`) and outputs retrieved data from a page. If content-type is `HTML`, existing tags are analysed, if content-type is `JSON`, exisiting arrays and objects are analysed.
